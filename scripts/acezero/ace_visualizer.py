...
# Platform-aware OpenGL backend selection
# EGL is Linux-only; on Windows, use osmesa or skip visualization
if sys.platform != 'win32':
    os.environ['PYOPENGL_PLATFORM'] = 'egl'
else:
    # On Windows, osmesa or default OpenGL might work, but pyrender often fails
    # We'll try to import pyrender, but allow failure
    os.environ['PYOPENGL_PLATFORM'] = 'osmesa'  # Attempt osmesa first

import logging
import math
import numpy as np

# Attempt to import pyrender with fallback
PYRENDER_AVAILABLE = False
try:
    import pyrender
    PYRENDER_AVAILABLE = True
except ImportError as e:
    logging.warning(f"pyrender not available (Windows/EGL issue): {e}")
    pyrender = None

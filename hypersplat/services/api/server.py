...
# hypersplat/services/api/server.py -> api -> services -> hypersplat -> root
GSPLAT_ROOT = CURRENT_DIR.parent.parent.parent
SCRIPTS_DIR = GSPLAT_ROOT / "scripts" / "pipeline"

# Single unified output directory
# Fix: Point to demo/output instead of local package dir
...
async def start_training(request: TrainRequest = TrainRequest()):
    ...
    try:
        ...
        config_override = {
            ...
            "means_lr": request.means_lr,
            "opacity_reg": request.opacity_reg,
            "scale_reg": request.scale_reg,
            "ssim_lambda": request.ssim_lambda,
            "random_bkgd": request.random_bkgd,
            "pose_opt": request.pose_opt,
            "app_opt": request.app_opt,
        }
...
async def start_viewer(req: ViewRequest = None):
    ...
    # We use a thread to pump the logs so they appear in the main console
